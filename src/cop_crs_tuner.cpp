...
int main(int argc, char *argv[]) {
  ...
  uint8_t max_exp = 10;//5;//10;
  uint8_t max_runs = 25;//10;//25;
  double_t MPS = pop_size / 2;
  for (uint8_t exp = 0; exp < max_exp; ++exp) {
    std::cout << "EXP: " << unsigned(exp) << std::endl;
...
//                    << unsigned(run) << std::endl;
          ...
          c = ga_cop(P[prob].cost_mat, P[prob].rewards,
              P[prob].max_cost, 0, P[prob].cost_mat.size() - 1, g,
              C[config].pop_size(), C[config].num_gen(),
              C[config].tour_size(), C[config].cx_rate(),
              C[config].mut_rate(), C[config].elitist_rate());
          auto end = std::chrono::high_resolution_clock::now();
          std::chrono::duration<double> diff = end - start;
          result = evaluateChromosome(
              c, P[prob].cost_mat, P[prob].rewards);
