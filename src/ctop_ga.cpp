...
namespace ctop_ga {
void
Gene::evaluate_gene(const Matrix<double> &cost_mat,
...
void Gene::mutate(Matrix<double> &cost_mat,
...
void Chromosome::evaluate_chromosome(Matrix<double> &cost_mat,
  ...
  std::pair<std::unordered_set<uint_fast32_t>::iterator, bool> insert_ret;
  for (std::vector<Gene>::iterator it = genes.begin(); it != genes.end();
       ++it) {
  ...
  std::vector<uint_fast32_t>
      visited_vertices(seen_vertices.begin(), seen_vertices.end());
  ...
  total_fitness =
      std::accumulate(genes.begin(),
                      genes.end(),
                      0.0,
                      [](double total, const Gene &g) {
                        return total + g.fitness;
                      });
}
  ...
  for (uint_fast32_t robot = 0; robot < num_robots; ++robot) {
    for (uint_fast32_t iter = 0; iter < 10; ++iter) {
      if (dis(g) < 0.9) {
        ...
        if (mutated.genes[robot].cost > max_cost
            || logically_equal(mutated.genes[robot].cost, max_cost)) {
          ...
          std::uniform_int_distribution<>
              dis(1, mutated.genes[robot].path.size() - 2);
          size_t rand_vertex_idx = dis(g);
          uint_fast32_t
              rand_vertex = mutated.genes[robot].path[rand_vertex_idx];
          ...
          double best_fitness =
              mutated.genes[robot].fitness;//mutated.total_fitness;//mutated.genes[robot].fitness;
//          double best_path_fitness = mutated.genes[robot].fitness;
          uint_fast32_t best_vertex = rand_vertex;
          uint_fast32_t
              prev_vertex = mutated.genes[robot].path[rand_vertex_idx - 1];
          uint_fast32_t
              next_vertex = mutated.genes[robot].path[rand_vertex_idx + 1];
...
//            seen_vertices.insert(i);
            if (tmp_c.cost < max_cost_v[robot]
                || logically_equal(tmp_c.cost, max_cost_v[robot])) {
...
//              }
              if (tmp_c.fitness
                  > best_fitness) {//total_fitness > best_fitness) {//tmp_c.fitness > best_fitness) {
...
//            seen_vertices.erase(rand_vertex);
          ...
          for (size_t i = 1; i < mutated.genes[robot].path.size(); ++i) {
            double travel_increase =
                cost_mat[mutated.genes[robot].path[i - 1]][vertex]
                    + cost_mat[vertex][mutated.genes[robot].path[i]]
                    - cost_mat[mutated.genes[robot].path[i
                        - 1]][mutated.genes[robot].path[i]];
            double fit =
            ...
            if (fit > best_fitness) {
              ...
              if (total_increase < max_cost_v[robot]
                  || logically_equal(total_increase, max_cost_v[robot])) {
          ...
          if (ins_pos > 0) {
            mutated.genes[robot].path.insert(
                mutated.genes[robot].path.begin() + ins_pos, vertex);
            mutated.genes[robot].cost += best_travel_increase + 1;
            mutated.genes[robot].evaluate_gene(cost_mat,
                                               rewards,
                                               free_vertices);
        ...
        for (size_t i = 1; i < mutated.genes[robot].path.size() - 1; ++i) {
          double travel_decrease =
              cost_mat[mutated.genes[robot].path[i
                  - 1]][mutated.genes[robot].path[i]]
                  + cost_mat[mutated.genes[robot].path[i]][mutated.genes[robot].path[
                      i + 1]]
                  - cost_mat[mutated.genes[robot].path[i
                      - 1]][mutated.genes[robot].path[i + 1]];
          ...
          for (size_t j = 0; j < free_vertices.size(); ++j) {
            if (cost_mat[mutated.genes[robot].path[i]][free_vertices[j]] < 2) {
              extras += std::exp((log(0.01) / 2)
                                     * cost_mat[mutated.genes[robot].path[i]][free_vertices[j]]);
            }
          }
          loss += extras;
        ...
        if (to_remove > 0) {
          ...
          mutated.genes[robot].path.erase(
              mutated.genes[robot].path.begin() + to_remove);
...
Chromosome::Chromosome(size_t num_vertices,
                       size_t num_genes,
  ...
  free_vertices =
      std::vector<uint_fast32_t>(all_vertices.begin(), all_vertices.end());
  free_vertices.erase(std::remove(free_vertices.begin(),
                                  free_vertices.end(),
                                  start_vertex));
  free_vertices.erase(std::remove(free_vertices.begin(),
                                  free_vertices.end(),
...
void Chromosome::GenerateInsertMoves(const Matrix<double_t> &cost_mat,
  ...
  for (const uint_fast32_t &v : free_vertices) {
    ...
    for (p = path.begin() + 1; p != path.end(); ++p) {
      ...
      double_t
          cost_increase = cost_mat[prev_vertex][v] + cost_mat[v][next_vertex]
          - cost_mat[prev_vertex][next_vertex] + 1;
...
//    std::cout << "insert candidates: " << insert_candidates.size() << " RIC: " << restricted_candidates_indexes.size() << std::endl;
    if (restricted_candidates_indexes.size() > 0) {
      std::uniform_int_distribution<size_t>
          index_dis(0, restricted_candidates_indexes.size() - 1);
      ...
      path_cost +=
          insert_candidates[restricted_candidates_indexes[idx]].cost_increase;
      free_vertices.erase(std::find(free_vertices.begin(),
                                    free_vertices.end(),
                                    insert_candidates[restricted_candidates_indexes[idx]].vertex));
    }
...
void Chromosome::GenerateGenes(Matrix<double_t> &cost_mat,
  ...
  std::list<uint_fast32_t>
      free_list(free_vertices.begin(), free_vertices.end());
  ...
  while ((free_list.size() > 0) && (list_it != free_list.end())) {
    ...
    for (free_it = free_vertices.begin(); free_it != free_vertices.end();
         ++free_it) {
      if (*free_it != *list_it) {
    ...
    for (gene_it = genes.begin(); gene_it != genes.end(); ++gene_it) {
      ...
      for (path_it = gene_it->path.begin() + 1; path_it != gene_it->path.end();
           ++path_it) {
        uint_fast32_t prev_vertex = *(path_it - 1);
        uint_fast32_t next_vertex = *path_it;
        double_t cost_increase =
            cost_mat[prev_vertex][*list_it] + cost_mat[*list_it][next_vertex]
                - cost_mat[prev_vertex][next_vertex] + 1;
        if (gene_it->cost + cost_increase
            < max_cost_v[0]) { // Ugly hack. Assumes all max costs are the same. Fixme!
  ...
  free_vertices =
      std::vector<uint_fast32_t>(free_list.begin(), free_list.end());
}
...
void Chromosome::insertGene(Gene &gene) {
  ...
  for (path_it = gene.path.begin(); path_it != gene.path.end(); ++path_it) {
    std::vector<uint_fast32_t>::iterator del_pos =
        std::find(free_vertices.begin(), free_vertices.end(), *path_it);
    if (del_pos != free_vertices.end())
      free_vertices.erase(del_pos);
...
//        uint_fast32_t next_vertex = available[rand_idx];
      ...
      if (seen_vertices.size() > 0) {
        ...
        for (avail_it = available.begin(); avail_it != available.end();
             ++avail_it) {
          double mean_dist = 0;
          double free_neighbours = 0;
          std::unordered_set<uint_fast32_t>::iterator used_it;
          for (used_it = seen_vertices.begin(); used_it != seen_vertices.end();
               ++used_it) {
        ...
        std::vector<double>::iterator
            neigh_weight_it = neighbour_weights.begin();
        double tot_combined = 0;
        for (; dist_weight_it != distance_weights.end();
               ++dist_weight_it, ++neigh_weight_it) {
          combined.push_back(*dist_weight_it / double_t(tot_dist)
                                 + *neigh_weight_it / double_t(tot_neighbours));
          tot_combined += *dist_weight_it / double_t(tot_dist)
              + *neigh_weight_it / double_t(tot_neighbours);
        }
        std::vector<double>::iterator combined_it;
        cdf.push_back(combined.front() / tot_combined);
        for (combined_it = combined.begin() + 1; combined_it != combined.end();
             ++combined_it) {
        ...
        std::vector<uint_fast32_t>::iterator avail_it;
        for (avail_it = available.begin() + 1; avail_it != available.end();
             ++avail_it) {
      ...
      uint_fast32_t
          next_vertex = available[std::distance(cdf.cbegin(), vertex_pos)];
      //==============================================================================================================
      double next_vertex_cost = cost_mat[path.back()][next_vertex] + 1;
      double next_vertex_end_cost =
          path_cost + next_vertex_cost + cost_mat[next_vertex][end_vertex];
      if (next_vertex_end_cost < max_cost
          || logically_equal(next_vertex_end_cost, max_cost)) {
...
double_t Chromosome::GenerateRandomPath(const Matrix<double_t> &cost_mat,
  ...
  while (!done) {
    ...
    double min_cost_to_end = path_cost + cost_mat[path.back()][to_insert]
        + cost_mat[to_insert][end_vertex] + 1;
    if (min_cost_to_end < max_cost
        || logically_equal(min_cost_to_end, max_cost)) {
...
void Chromosome::removeCommonVertices(const Gene &gene) {
  Path::const_iterator path_it;
  for (path_it = gene.path.begin() + 1; path_it != gene.path.end() - 1;
       ++path_it) {
    std::vector<Gene>::iterator genes_it;
    for (genes_it = genes.begin(); genes_it != genes.end(); ++genes_it) {
      Path::iterator pos =
          std::find(genes_it->path.begin(), genes_it->path.end(), *path_it);
      if (pos != genes_it->path.end())
        genes_it->path.erase(std::find(genes_it->path.begin(),
                                       genes_it->path.end(),
                                       *path_it));
    }
  }
...
Chromosome generate_chromosome(Matrix<double> &cost_mat,
  ...
  if (method == "NNGRASP") {
    for (int i = 0; i < n_agents; ++i) {
      c.genes[i].cost =
          c.GenerateNNGRASPPath(cost_mat,
                                rewards,
                                max_cost_v[i],
                                idx_start,
                                idx_finish,
...
void construct_offspring(Chromosome &offspring,
  ...
  if (tmp_parent_1.genes.size() == 0 || tmp_parent_2.genes.size() == 0) {
    std::cout << " one of the parents didn't have any genes" << std::endl;
    if (tmp_parent_1.genes.size() > 0) {
      offspring = tmp_parent_1;
    } else if (tmp_parent_2.genes.size() > 0) {
      offspring = tmp_parent_2;
    } else {
      std::cout << "Both parents were empty. Failing..." << std::endl;
      exit(1);
    }
    return;
  }
  ...
  // First construct
  while (offspring.genes.size() < num_robots) {
    Gene to_insert;
    if (dis(g) < 0.5) {
      // Choose best gene from parent 1.
      // Remove that gene from parent 1.
      if (tmp_parent_1.genes.size() == 0) {
        std::cout << tmp_parent_1.genes.size() << " "
                  << tmp_parent_2.genes.size() << " " << offspring.genes.size()
                  << " " << num_robots << std::endl;
      } else {
        ...
        std::sort(tmp_parent_2.genes.begin(),
                  tmp_parent_2.genes.end(),
                  sortRuleLambda);
      }
    } else {
      // Choose best gene from parent 2.
      // Remove that gene from parent 2.
      if (tmp_parent_2.genes.size() == 0) {
        std::cout << tmp_parent_1.genes.size() << " "
                  << tmp_parent_2.genes.size() << " " << offspring.genes.size()
                  << " " << num_robots << std::endl;
      } else {
        ...
        std::sort(tmp_parent_1.genes.begin(),
                  tmp_parent_1.genes.end(),
                  sortRuleLambda);
      }
    }
  }
  //TODO: Then check if you can improve the offspring genes by inserting free vertices.
  offspring.evaluate_chromosome(cost_mat, rewards, max_cost_v);
  std::sort(offspring.genes.begin(), offspring.genes.end(), sortRuleLambda);
#if 0
  std::vector<Gene>::iterator gene_it;
  for (gene_it = offspring.genes.begin(); gene_it != offspring.genes.end(); ++gene_it) {
    std::uniform_real_distribution<> greediness_dis(0, 1);
    double_t greediness = greediness_dis(g);
    std::vector<InsertMove> insert_candidates;
    insert_candidates.reserve(offspring.free_vertices.size());
    double_t min, max;
    offspring.GenerateInsertMoves(cost_mat,
                                  rewards,
                                  max_cost_v[0],
                                  gene_it->path,
                                  gene_it->cost,
...
//    std::cout << "insert candidates: " << insert_candidates.size() << " RIC: " << restricted_candidates_indexes.size() << std::endl;
      if (restricted_candidates_indexes.size() > 0) {
        ...
        size_t idx = index_dis(g);
        gene_it->path.insert(std::find(gene_it->path.begin(),
                                       gene_it->path.end(),
                                       insert_candidates[restricted_candidates_indexes[idx]].next_vertex),
                             insert_candidates[restricted_candidates_indexes[idx]].vertex);
        gene_it->cost += insert_candidates[restricted_candidates_indexes[idx]].cost_increase;
        offspring.free_vertices.erase(std::find(offspring.free_vertices.begin(),
                                                offspring.free_vertices.end(),
                                                insert_candidates[restricted_candidates_indexes[idx]].vertex));
      }
      offspring.GenerateInsertMoves(cost_mat,
                                    rewards,
  ...
  std::sort(offspring.genes.begin(), offspring.genes.end(), sortRuleLambda);
#endif
  if (offspring.genes.size() == 0) {
    std::cout << "Offspring has no genes!" << std::endl;
    exit(0);
  }
}
...
//  std::vector<Chromosome> family = {parent_1, parent_2, c1, c2};
//  std::sort(family.begin(), family.end(), sortRuleLambdaCr);
//  c1 = family[0];
//  c2 = family[1];
}
...
void par_cx(std::vector<size_t> indices,
  ...
  for (size_t i = 0; i < indices.size(); i = i + 2)
    cx(pop[indices[i]], pop[indices[i + 1]], cost_mat, max_cost_v, rewards, g);
}
...
Chromosome ga_ctop(Matrix<double> &cost_mat,
  ...
  */
  ...
  for (uint_fast32_t i = 0; i < pop_size; ++i) {
    Chromosome c = generate_chromosome(cost_mat,
                                       max_cost_v,
                                       rewards,
                                       idx_start,
                                       idx_finish,
...
//      cx(new_pop[indices[i]], new_pop[indices[i + 1]], cost_mat, max_cost_v, rewards, g);
//    }
    if (cx_rate != 0) {
      int num_individuals = std::ceil(pop_size * cx_rate);
      if (num_individuals % 2)
...
//    if(logically_equal(best.total_fitness, pop[0].total_fitness)){
//      ++num_best;
//    } else {
//      best = pop[0];
//      num_best = 0;
