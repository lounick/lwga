...
class Configuration {
 public:
  ...
  void updateRating() {
    ...
    double_t phi2 = std::pow(phi, 2);
    double_t e0 = std::exp(0);
    double_t t2 = std::pow(tau, 2);
    double_t sigma2 = std::pow(volatility_, 2);
    double_t x0 =
        (e0 * (delta2 - phi2 - v - e0)) / (2 * std::pow((phi2 + v + e0),2)) -
        (0 - std::log(sigma2)) / t2;
    volatility_ = std::exp(x0/2.0);*/
    double_t sigma2 = std::pow(volatility_, 2);
...
//      double_t mui = mu(games_[game].opponent_rating);
//      double_t phii = phi(games_[game].opponent_rating_deviation);
//      double_t E = E(mu, mui, phii);
//      for (size_t prob = 0; prob < games_[game].results.size(); ++prob) {
//        for(size_t iter = 0; iter < games_[game].results[prob].size(); ++iter) {
//          mu_prime += g(phii)*(games_[game].results[prob][iter] - E);
//        }
//      }
//    }
...
Problems generateProblems() {
  ...
  Problem prob5x5random3robs = {
      ...
      std::vector<double>(3, 3 * ((random5_objective + 25.0) / 3) / 4.0)};
  Problem prob5x5canonical4robs = {
      canonical5_costmat, rewards5x5,
      std::vector<double>(4, 3 * ((canonical5_objective + 25.0) / 4) / 4.0)};
