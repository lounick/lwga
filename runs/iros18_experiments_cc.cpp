...
int main(int argc, char *argv[]) {
  ...
  Vector<double_t> rhos = {0.5};
//  Vector<double_t> cost_coef = {0.25};
  ...
  for (double_t rho:rhos) {
    ...
    for (double_t cc : cost_coef) {
      ...
      util_results_file.open(
          "/tmp/cc_" + std::to_string(size) + "x" + std::to_string(size)
              + to_string_with_precision(cc)+"_util.csv");
      ...
      time_results_file.open(
          "/tmp/cc_" + std::to_string(size) + "x" + std::to_string(size)
              + to_string_with_precision(cc)+"_time.csv");
      ...
      best_worst_file.open(
          "/tmp/cc_" + std::to_string(size) + "x" + std::to_string(size)
              + to_string_with_precision(cc)+"_best_worst.txt");
